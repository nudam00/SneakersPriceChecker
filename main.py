...
from add import add_site, best_site, get_settings
...
if __name__ == "__main__":
    ...
    with sync_playwright() as p:
        ...
        # Preparing
        alias = Alias(get_settings("alias_username"), get_settings("alias_password"))
        hypeboost = Hypeboost()
        klekt = Klekt()
        sneakit = Sneakit(get_settings("email"), get_settings("sneakit_password"))
        stockx = StockX(
            get_settings("email"),
            get_settings("stockx_password"),
            page,
        ...
        for i in range(len(pd.ExcelFile("input/stock.xlsx").sheet_names)):
            ...
            for index, row in excel.iterrows():
                ...
                if size == new_row["Size"] and sku == new_row["SKU"]:
                    ...
                    stockx_data = stockx.get_price(sku, sizes_converter.stockx())
                    ...
                    new_row = {
                        ...
                        "SKU": sku,
                        "Size": str(row["size"]),
                        "StockX_payout": str(stockx_price).replace(".", ","),
                        "Alias_payout": str(alias_price).replace(".", ","),
                        "Best_site": site,
                        "Best_price": str(best_price).replace(".0", ",0"),
            ...
            with pd.ExcelWriter(
                "output/prices.xlsx", engine="openpyxl", mode="a"
            ) as writer:
